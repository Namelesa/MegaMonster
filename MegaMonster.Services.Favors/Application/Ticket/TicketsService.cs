...
namespace MegaMonster.Services.Favors.Application.Ticket;
    ...
    IPublishEndpoint publishEndpoint)
{
    ...
    public async Task<IEnumerable<Core.Ticket.Ticket>> GetAllTicketsByStatus(string status)
    {
    ...
    public async Task<ResultOperation> BuyTickets(List<Core.Ticket.Ticket> tickets, string userName, Guid userId, string paymentType)
    {
        if (tickets.Count == 0)
        ...
        var purchasedTickets = new List<Core.Ticket.Ticket>();
